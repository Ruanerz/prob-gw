import { getLegendaryItem, isLegendaryItem, isBasicMaterial, LEGENDARY_ITEMS } from './data/legendaryItems1gen.js';
...
class LegendaryCraftingApp {
  constructor() {
    ...
    this.quickLoadButtons = {
      ...
      'btnFrostfang': { id: 'btnFrostfang', itemId: '30684', itemName: 'Colmilloescarcha' },
  ...
  _handleQuickLoad(buttonId) {
    ...
    // Cargar el ítem por su ID
    this.loadLegendaryItemById(buttonData.itemId);
  }
  ...
  /**
   * Carga un ítem legendario por su ID
   * @param {string} itemId - ID del ítem a cargar
   */
  async loadLegendaryItemById(itemId) {
    if (!itemId) {
      this.showError('ID de ítem no proporcionado');
      return;
    }
    ...
    try {
      const itemData = getLegendaryItem(parseInt(itemId));
      
      if (!itemData) {
        throw new Error(`No se encontró información local para el ítem con ID: ${itemId}`);
      }
      
      // Actualizar el botón activo basado en el ID del ítem
      const buttonId = this._findButtonIdByItemId(itemId);
      if (buttonId) {
        const button = document.getElementById(buttonId);
        this._setActiveButton(button);
      }
  ...
  async renderIngredient(ingredient, container, depth = 0) {
    ...
    try {
      ...
      if (isDon || isVial || isEstatua) {
        ...
        showPrice = isBasicMaterial(ingredient.id) || ingredient.isPriceLoaded() || 
                  (isLegendary && (hasBuyPrice || hasSellPrice));
  ...
  renderProfitSummary(sellPrice, buyPrice, craftingCost) {
    ...
    profitEl.innerHTML += `
      <div class="summary-note">
        ...
        <small>Nota 2: El precio para el trébol místico se ocupa la cantidad promedio que se requiere para obtener los 77 tréboles, por lo que según el RNG puede variar.</small>
      </div>`;
